...
import { researchChosenSetups } from '@/api/researchChosenSetups'
import {
...
import { ResearchChosenSetup } from '@/components/research/ResearchChosenSetup'
...
import {
  ...
  researchResultRole,
  strategyRuleSummary,
} from '@/components/research/researchPresentation'
...
} from '@/hooks/useResearchNavigation'
        ...
        >
          <span className="min-w-0">
            ...
            <span className="mt-1 block text-xs text-muted-foreground">
              {researchResultRole(job)}
              {researchResultPeriod(job) ? ` · ${researchResultPeriod(job)}` : ''}
              {job.display?.setup ? ` · Setup ${job.display.setup.number}` : ''}
              {' · '}
              {date(job.created_at)}
            </span>
          </span>
...
export default function ResearchLibrary() {
  ...
  // Observe the existing child queries without issuing another request.
  ...
  const jobState = useQuery({
    queryKey: ['portfolio-job', owner, job],
    queryFn: ({ signal }) => portfolioResearch.job(job!, signal),
    enabled: false,
  })
  useResearchNavigation({
    owner,
    experimentId: id,
    targetRef: navigationRef,
    ready:
      chartinkImport !== null ||
      Boolean(id && experimentState.error) ||
      ((!id || Boolean(experimentState.data && !experimentState.isFetching)) &&
        (!job || Boolean(jobState.data || jobState.error))),
  })
  return (
    <div ref={navigationRef}>
      {chartinkImport !== null ? (
        <ChartinkImport key={chartinkImport} requestId={chartinkImport} owner={owner} />
      ) : id ? (
        <ExperimentLoader key={`${owner}:${id}`} id={id} owner={owner} />
      ) : job ? (
        <PreviousReport key={`${owner}:${job}`} owner={owner} />
      ) : (
        <LibraryIndex key={owner} owner={owner} params={params} setParams={setParams} />
      )}
    </div>
  )
}
...
function LibraryIndex({
  ...
  async function create() {
    await action(async () => {
      ...
      draft.portfolio.name = name.trim() || 'Untitled experiment'
      const experiment = await researchLibrary.create({
        name: draft.portfolio.name,
        draft,
      })
  ...
  async function chooseSource(source: PortfolioSource) {
    await action(async () => {
      ...
      draft.portfolio.name = draft.portfolio.strategies[0].name
      const experiment = await researchLibrary.create({
        name: draft.portfolio.name,
        draft,
      })
...
function ExperimentWorkspace({ initial, owner }: { initial: ResearchExperiment; owner: string }) {
  ...
  const returnResearch = researchBackParams(params.get('return_research'), server.id)
  ...
  const view = [
    ...
    ? requestedView
    : 'overview'
  const primaryView = ['shortlist', 'comparisons', 'decisions'].includes(view)
    ? 'review'
    : view === 'setups'
      ? 'setup'
      : view
  ...
  useEffect(() => {
    ...
    const revealActive = () => {
      const selected = nav.querySelector<HTMLElement>(
        `[data-experiment-view="${primaryView}"][aria-current="page"]`
      )
      if (!selected) return
  ...
  function changeView(nextView: string, job?: string, returning = false) {
    const next = new URLSearchParams(params)
    ...
    next.delete('return_research')
    next.delete('matched_study_candidate')
    next.delete('matched_compare_request')
    ...
    setParams(
      next,
      returning ? { state: researchNavigationReturn(owner, server.id, next) } : undefined
    )
  }
  function library() {
    const next = new URLSearchParams(params)
    for (const key of [
      ...
      'return_shortlist',
      'return_research',
      'matched_study_candidate',
      'matched_compare_request',
      'comparison',
      'comparison_member',
      'comparison_offset',
      'compare_candidates',
  ...
  const recentRuns = server.jobs
    .filter((job) => ['portfolio_backtest', 'portfolio_optimize'].includes(job.kind ?? ''))
    .sort((left, right) => jobCreatedAt(right) - jobCreatedAt(left))
  const continuation =
    recentRuns.find(active) ??
    recentRuns.find((job) => job.status === 'completed' || (stopped(job) && job.resumable)) ??
    recentRuns.find(stopped)
  ...
  return (
    <ResearchWorkspaceMutations.Provider
      value={{
        beforeChange: async () => {
          await operation(() => model.flush())
        },
        afterChoice: async () => {
          await operation(() => model.refreshMetadata())
        },
      }}
    >
      <section
      ...
      >
        ...
        >
          {(['overview', 'setup', 'backtests', 'studies', 'review'] as const).map((item) => (
            <Button
              key={item}
              variant={primaryView === item ? 'secondary' : 'ghost'}
              size="sm"
              className="shrink-0"
              aria-current={primaryView === item ? 'page' : undefined}
              data-experiment-view={item}
              disabled={busy}
              onClick={() => {
                void action(async () => {
                  await model.flush()
                  changeView(item === 'review' ? 'decisions' : item)
                })
              }}
            >
        ...
        {!jobId && ['review', 'setup'].includes(primaryView) && (
          <nav
            aria-label={primaryView === 'review' ? 'Review' : 'Setup'}
        ...
        {jobId &&
          (params.get('report') !== 'best' || params.has('return_shortlist') || returnResearch) && (
                ...
                onClick={() =>
                  returnResearch
                    ? setParams(returnResearch, {
                        state: researchNavigationReturn(owner, server.id, returnResearch),
                      })
                    : params.get('return_shortlist')
                      ? (() => {
                          ...
                          next.delete('report')
                          setParams(next, {
                            state: researchNavigationReturn(owner, server.id, next),
                          })
                        })()
                      : params.get('return_job')
                        ? changeView('studies', params.get('return_job')!, true)
                        : changeView(view === 'studies' ? 'studies' : 'backtests', undefined, true)
                }
              >
                <ArrowLeft className="mr-2 size-4" />
                {returnResearch
        ...
        {!jobId && view === 'overview' && (
          <div className="space-y-7">
            <ResearchChosenSetup
              experimentId={server.id}
              readOnly={server.archived || model.state === 'conflict'}
              onUse={(request, signal) =>
                operation(async () => {
                  await model.flush()
                  if (signal.aborted) return
                  const next = await researchChosenSetups.use(server.id, request, signal)
                  if (signal.aborted) return
                  model.accept(next.experiment)
                  changeView('setup')
                })
              }
              onReplay={(choice, revision, requestId, signal) =>
                operation(async () => {
                  await model.flush()
                  if (signal.aborted) return
                  const next = await researchChosenSetups.replay(
                    server.id,
                    { choice_id: choice.id, revision, request_id: requestId },
                    signal
                  )
                  if (signal.aborted) return
                  model.accept(next.experiment)
                  changeView('backtests', next.job.id)
                })
              }
            />
            {server.notes && (
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 border-b pb-6">
              ...
              <Button
                disabled={busy || (!continuation && server.archived)}
                onClick={() =>
                  continuation
                    ? changeView(
                        continuation.kind === 'portfolio_optimize' ? 'studies' : 'backtests',
                        continuation.id
                      )
                    : changeView('setup')
                }
                ...
                {continuation
                  ? active(continuation)
                    ...
                    : stopped(continuation)
                      ? continuation.resumable && !server.archived
                        ? 'Resume run'
                        : 'View run'
                      : 'Continue research'
                  : draft.portfolio.strategies.length
                    ? 'Continue setup'
                    : 'Add signals'}
              </Button>
            </div>
            ...
            {displayedVersions.map((version) => (
              ...
              >
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {version.draft.portfolio.strategies.length === 1
                      ? strategyRuleSummary(
                          version.draft.portfolio.strategies[0],
                          version.draft.optimizing
                        )
                      : version.draft.portfolio.strategies
                          .map((strategy) => strategy.name)
                          .join(' · ')}
                  </p>
                  <details className="mt-2 text-xs">
                    <summary className="cursor-pointer text-muted-foreground">View rules</summary>
                    <ul className="mt-2 space-y-2">
                      {version.draft.portfolio.strategies.map((strategy) => (
                        <li key={strategy.id}>
                          {strategy.name}: {strategyRuleSummary(strategy, version.draft.optimizing)}{' '}
                          · Allocation {strategy.allocation_pct}% · Trade size{' '}
                          {strategy.config.order_size_pct}% of allocation · Costs{' '}
                          {strategy.config.cost_bps} bps/side · Slippage{' '}
                          {strategy.config.slippage_bps} bps/side
                        </li>
                      ))}
                    </ul>
                  </details>
